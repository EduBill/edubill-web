...
function handleSubmit() {
  ...
  //emit('selectedClass', selectedClass);
...
//const observer = intersectionObserver(hasMoreData, page, getClassInfo);
...
async function getClassInfo() {
  ...
  if (Array.isArray(res.data.content)) {
    ...
    console.log('출력 : ', classList.value);
  }
}
function handleChecked(event: Event) {
  ...
  // const id = target.id;
  // if (isChecked) {
  //   selectedClass.value.push(Number(id));
  // }
}
onMounted(async () => {
  await getClassInfo();
